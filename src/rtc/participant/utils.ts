import { compareVersions, getBrowser, type VideoEncoding } from "livekit-client";
import { getReactNativeOs, isFireFox, isReactNative, isSafari, isSVCCodec, unwrapConstraint } from "../utils";
import { FFCVideoPresets, FFCVideoPresets43, FFCScreenSharePresets, FFCVideoPreset, FFCTrackPublishOptions, type FFCBackupVideoCodec, type FFCVideoCodec, type FFCVideoEncoding } from "../track/options";
...
import { FFCTrackInvalidError } from "../../errors";
...
export function mediaTrackToLocalTrack(
  ...
  switch (mediaStreamTrack.kind) {
    ...
    default:
      throw new FFCTrackInvalidError(`unsupported FFCTrack type: ${mediaStreamTrack.kind}`);
  }
}

/* @internal */
export const presets169 = Object.values(FFCVideoPresets);

/* @internal */
export const presets43 = Object.values(FFCVideoPresets43);

/* @internal */
export const presetsScreenShare = Object.values(FFCScreenSharePresets);

/* @internal */
export const defaultSimulcastPresets169 = [FFCVideoPresets.h180, FFCVideoPresets.h360];

/* @internal */
export const defaultSimulcastPresets43 = [FFCVideoPresets43.h180, FFCVideoPresets43.h360];

/* @internal */
export const computeDefaultScreenShareSimulcastPresets = (fromPreset: FFCVideoPreset) => {
...
export function computeVideoEncodings(
  ...
  width?: number,
  height?: number,
  options?: FFCTrackPublishOptions,
  ...
  const original = new FFCVideoPreset(
    width,
    height,
  ...
  let presets: Array<FFCVideoPreset> = [];
  if (isScreenShare) {
    presets =
  ...
  let midPreset: FFCVideoPreset | undefined;
  if (presets.length > 0) {
...
export function computeTrackBackupEncodings(
  ...
  videoCodec: FFCBackupVideoCodec,
  opts: FFCTrackPublishOptions,
) {
  ...
  if (track.source === FFCTrack.Source.ScreenShare && opts.simulcast) {
    opts.simulcast = false;
  }
  const encodings = computeVideoEncodings(
    track.source === FFCTrack.Source.ScreenShare,
    width,
    height,
    opts,
  );
...
/** @internal */
export function sortPresets(presets: Array<FFCVideoPreset> | undefined) {
  if (!presets) return;
...
export function getDefaultDegradationPreference(track: FFCLocalVideoTrack): RTCDegradationPreference {
